...
const navItems = [
  ...
  { key: "nav.requirements", href: "/requirements", icon: ClipboardList },
  ...
  { key: "nav.deadlines", href: "/deadlines", icon: Clock },
  { key: "nav.notes", href: "/notes", icon: StickyNote },
  ...
  return (
    <aside
      className={cn(
        ...
        // Responsive positioning: sliding drawer on mobile, static on desktop
    ...
    >
      ...
      <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        {navItems.map(({ key, href, icon: Icon }) => (
          <NavLink
            ...
            title={showCollapsed ? t(key) : undefined}
          >
      ...
      <div className="px-3 pb-5 space-y-1 border-t border-border pt-4 mt-auto">
        ...
        <NavLink
          to="/settings"
          onClick={() => setIsMobileOpen(false)}
          className={cn("sidebar-link", showCollapsed && "md:justify-center md:px-0")}
          title={showCollapsed ? t("common.helpCenter") : undefined}
        >
          <HelpCircle size={18} className={showCollapsed ? "mx-auto" : ""} />
          <span className={cn(showCollapsed && "md:hidden")}>{t("common.helpCenter")}</span>
        </NavLink>
        ...
        <button
          ...
          title={showCollapsed ? t("common.logout") : undefined}
        >
          <LogOut size={18} className={showCollapsed ? "mx-auto" : ""} />
          <span className={cn(showCollapsed && "md:hidden")}>{t("common.logout")}</span>
        </button>
      </div>
    </aside>
  );
